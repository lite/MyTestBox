...
class MitmProxy(HTTPServer):
    ...
    def __init__(self, server_address=('', 8086), RequestHandlerClass=ProxyHandler, bind_and_activate=True, ca_file='ca.pem'):
...
class SlimInterceptor(RequestInterceptorPlugin, ResponseInterceptorPlugin):
    def do_request(self, data):
        print '>> %s' % repr(data[:100])
        # Accept-Encoding: gzip,deflate,sdch\r\n
        return re.sub("Accept-Encoding:.+\r\n", "Accept-Encoding: \r\n", data)

    def do_response(self, data):
        print '<< %s' % repr(data[:100])
        return self.slim(data)
        
    def slim(self, raw_data):
        headers, message = raw_data.split("\r\n\r\n", 1)
        print headers
        if "http://www.bjjtgl.gov.cn/publish/portal0/tab72/Default.htm" in headers:
            return self.slim_response(headers, message)
        elif "http://sslk.bjjtgl.gov.cn/jgjww/wzcx/wzcx_preview.jsp" in headers:
            return self.slim_response(headers, message)
        else:
            return raw_data

    def slim_response(self, headers, message):
        message = self.slim_html(message)
        re.sub("Content-Length: \d+\r\n", "Content-Length: %d\r\n"%(len(message)), headers)
...
if __name__ == '__main__':
    ...
    proxy.register_interceptor(SlimInterceptor)
    try:
        proxy.serve_forever()
